...
public:
  ...
  static uint32_t codeConfig(SamplePeriod sp    = SP_SHORT,
  ...
  {
    if(ichan<8)return codeSelectChannel(ichan);
    else if(ichan==9)return codeSelectRefMid();
    else if(ichan==10)return codeSelectRefM();
    else return codeSelectRefP();
  }
