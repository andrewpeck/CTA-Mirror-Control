...
#ifndef VERDEXXM4_HPP
#define VERDEXXM4_HPP
...
public:
  ...
  static int pin60ToGPIO(unsigned ipin60 /* STARTS FROM ONE */)
  {
    static const int igpio[] =  
      {  -1,   67,   77,   61,  30,   66,   -1,   71,   70,   69,
	 -1,   46,   60,  100,   9,   75,   17,   58,  118,  117,
	 19,   87,   13,   41,  -1,   39,   86,   -1,   -1,   -1,
	 -1,   44,   42,   45,  43,   16,   -1,  101,   27,   64,  
	 74,   -1,   29,   14,  11,   31,   62,   47,  113,   59,
	 68,   63,   65,   34,  72,   73,   76,   28,   -1,   -1 };
    if((ipin60>0)&&((--ipin60)<N(igpio)))return igpio[ipin60];
    else return -1;
  }
};
...
public:
  ...
  static unsigned nGPIO() { return 119; /* 121; */ }
  static bool hasGPIORegister(const unsigned ipin) { return ipin<nGPIO(); }
  static bool hasGPIOPin(const unsigned ipin)
  {
    if((ipin==2)||((ipin<=8)&&(ipin>=5))||(ipin>=nGPIO()))return false;
    return true;
  }
  ...
  enum GPIODir { GPIODIR_IN = 0, GPIODIR_OUT };
  enum GPIOFn { GPIOFN_IO = 0, GPIOFN_ALT1, GPIOFN_ALT2, GPIOFN_ALT3 };

  // --------------------------------------------------------------------------
  // SSP Definitions
  // --------------------------------------------------------------------------

  enum SSPFrameFormat { SSP_FF_SPI, SSP_FF_TISSP, SSP_FF_MICROWIRE, 
			SSP_FF_PSP };
  enum SSPClockSelect { SSP_CS_INTERNAL, SSP_CS_NETWORK, SSP_EXTERNAL,
			SSP_CS_AUDIO };

  enum SSPPSPSerialClockMode { SSP_PSP_SCMODE_FRL, SSP_PSP_SCMODE_RFL,
			       SSP_PSP_SCMODE_RFH, SSP_PSP_SCMODE_FRH };
...
class VerdexXM4: public VerdexXM4Base, public REV, public REG
{
public:
  VerdexXM4(): VerdexXM4Base(), REV(), REG() { }
  ~VerdexXM4() { }
  ...
  {
    if(enable)*REG::ptrClockEnable() |= (0x1<<ibit);
    else *REG::ptrClockEnable() &= ~(0x1<<ibit);
  }

  void clockClearEnableBit(unsigned ibit) { clockSetEnableBit(ibit,false); }

  void clockEnableSSP1(bool enable = true) { clockSetEnableBit(23,enable); }
  void clockEnableSSP2(bool enable = true) { clockSetEnableBit(3,enable); }
  void clockEnableSSP3(bool enable = true) { clockSetEnableBit(4,enable); }
  void clockEnablePWM(bool enable = true) 
  {
    clockSetEnableBit(0,enable); 
    clockSetEnableBit(1,enable); 
  }

  void clockEnableSSP(unsigned issp, bool enable = true)
  {
    if(issp==0)clockEnableSSP1(enable);
    else if(issp==1)clockEnableSSP2(enable);
    else clockEnableSSP3(enable);
  }

  void clockDisableSSP1() { clockEnableSSP1(false); }
  void clockDisableSSP2() { clockEnableSSP2(false); }
  void clockDisableSSP3() { clockEnableSSP3(false); }
  void clockDisablePWM() { clockEnablePWM(false); }

  void clockDisableSSP(unsigned issp) { clockEnableSSP(issp,false); }
  ...
  bool clockIsEnabledSSP1() { return clockIsBitEnabled(23); }
  bool clockIsEnabledSSP2() { return clockIsBitEnabled(3); }
  bool clockIsEnabledSSP3() { return clockIsBitEnabled(4); }
  bool clockIsEnabledPWM() 
  {
    return clockIsBitEnabled(0)&&clockIsBitEnabled(1); 
  }

  bool clockIsEnabledSSP(unsigned issp)
  {
    if(issp==0)return clockIsEnabledSSP1();
    else if(issp==1)return clockIsEnabledSSP2();
    else return clockIsEnabledSSP3();
  }
  ...
  {
    //std::cout << "W: " << ipin << ' ' << level << std::endl;
    if(level)REG::gpioSetLevel(ipin);
    else REG::gpioClrLevel(ipin);
  }
  ...
  GPIODir gpioGetDirection(const unsigned ipin)
  {
    ...
    uint32_t val = *reg;
    if(val & Bits::mask1Bit(ipin))return GPIODIR_OUT;
    else return GPIODIR_IN;
  }
  ...
  void gpioSetDirection(const unsigned ipin, const GPIODir dir)
  {
    volatile uint32_t* reg = REG::ptrGPIODirection(ipin);
    uint32_t val = *reg;
    if(dir==GPIODIR_OUT)val |= Bits::mask1Bit(ipin);
    else val &= ~Bits::mask1Bit(ipin);
    *reg = val;
  }
  ...
  // --------------------------------------------------------------------------
  // PWM utility functions
  // --------------------------------------------------------------------------

  void pwmSetPrescale(const unsigned ipwm, const unsigned prescale)
  {
    *REG::ptrPWMControl(ipwm) = prescale & 0x3F;
  }

  uint32_t pwmGetPrescale(const unsigned ipwm)
  {
    return *REG::ptrPWMControl(ipwm) & 0x3F;
  }

  float pwmGetScaledClockTimeNS(const unsigned ipwm)
  {
    return float(pwmGetPrescale(ipwm)+1)*76.9;
  }

  void pwmSetDutyCycle(const unsigned ipwm, const unsigned duty_cycle, 
		       const bool full_duty)
  {
    *REG::ptrPWMDutyCycle(ipwm) = (full_duty?0x0400:0x0000)|(duty_cycle&0x03FF);
  }

  bool pwmGetFullDutyFlag(const unsigned ipwm)
  {
    return *REG::ptrPWMDutyCycle(ipwm)&0x0400;
  }

  uint32_t pwmGetDutyCycle(const unsigned ipwm)
  {
    return *REG::ptrPWMDutyCycle(ipwm)&0x03FF;
  }

  bool pwmIsZeroDuty(const unsigned ipwm)
  {
    return (*REG::ptrPWMDutyCycle(ipwm)&0x07FF)==0;
  }

  float pwmGetDutyTimeNS(const unsigned ipwm)
  {
    return float(pwmGetDutyCycle(ipwm)*(pwmGetPrescale(ipwm)+1))*76.9;
  }  

  void pwmSetPeriod(const unsigned ipwm, const unsigned period)
  {
    *REG::ptrPWMPeriod(ipwm) = period&0x03FF;
  }

  uint32_t pwmGetPeriod(const unsigned ipwm)
  {
    return *REG::ptrPWMPeriod(ipwm)&0x03FF;
  }

  float pwmGetPeriodTimeNS(const unsigned ipwm)
  {
    return float((pwmGetPeriod(ipwm)+1)*(pwmGetPrescale(ipwm)+1))*76.9;
  }
  
  bool pwmIsFullDuty(const unsigned ipwm)
  {
    uint32_t dc = *REG::ptrPWMDutyCycle(ipwm);
    return (dc&0x0400)||((dc&0x03FF)>pwmGetPeriod(ipwm));
  }

  float pwmGetDutyFraction(const unsigned ipwm)
  {
    uint32_t dc = *REG::ptrPWMDutyCycle(ipwm);
    uint32_t pe = pwmGetPeriod(ipwm);
    if((dc&0x0400)||((dc&0x03FF)>pe))return 1.0;
    return float(dc&0x03FF)/float(pe+1);
  }
  ...
  void sspDisable(unsigned issp) { sspEnable(issp,false); }

  void sspClearStatusBits(unsigned issp)
  {
    *REG::ptrSSPStatus(issp) = 
      (0x1<<23)|(0x1<<21)|(0x1<<20)|(0x1<<19)|(0x1<<18)|(0x1<<7);
  }
...
template<typename REG, typename REV> bool VerdexXM4<REG,REV>::
	     ...
	     uint32_t psp_flags)
{
  ...
  *REG::ptrSSPPSPConfig(issp) = 0;
  ...
  // SSCR0 : BIT 31, 26:24
  if((flags & SSP_FLAG_NETWORK_MODE)
     &&((frame_format == SSP_FF_TISSP)||(frame_format == SSP_FF_PSP)))
    {
      sscr0 |= (0x1<<31);
  ...
  if(frame_format == SSP_FF_SPI)
    {
      if(flags & SSP_FLAG_SPH)sscr1 |= (0x1<<4);
      if(flags & SSP_FLAG_SPO)sscr1 |= (0x1<<3);
    }
